...
class RobotAgent():
    ...
    def __init__(self, robot_name, current_position:str, navigate_position:list, config_path):
        ...
        task_thread.start()
        self.tools_list = [self.Load(self)]
    ...
    def load_object(self, object:str):
        print(">>>>>>>> Task load_object started <<<<<<<<")
        return True, 'Robot arm successfully grasped the object and placed it.'
    
    class Load(BaseTool):
        name = 'load'
        description = 'load the object around the robot arm to a mobile robot nearby'
        parameters = [{
            'name': 'object_name',
            'type': 'string',
            'description': 'The kind of the object to be loaded, A or B',
            'required': True
        }]
        ...
        def call(self, params: str, **kwargs) -> str:
            ...
            # 在实际应用中，这里会调用机械臂的抓取API
            # 当前为模拟实现
            status, description = self.robot.load_object(object_name)
            result = {
            ...
            print(">>>>>>>> Task load executed <<<<<<<<")
            
            return json.dumps(result, ensure_ascii=False)
        
    def set_scene_properties(self, item:str, position=None, objects:List=None):
        """
        Set the scene properties.
        """
        ret = True
        if objects:
            ret = self.master_memory.add_object_to_item(item, objects)
        if position:
            ret = self.master_memory.update_item_property(item, 'position', position)
        return ret
...
if __name__ == '__main__':
    robot_agent = RobotAgent('robot1', 'Dock1', ['Dock1'], 'robot_arm/robot_agent_config.yaml')