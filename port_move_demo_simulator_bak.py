...
class Arm:
    def __init__(self, name: str, base_pos: Tuple[int,int], dock_rect: pygame.Rect, queue: List[CargoItem]):
        self.name=name; self.base_pos=base_pos; self.dock_rect=dock_rect; self.queue=queue
    ...
    def load(self, kind: Optional[str]=None) -> Optional[CargoItem]:
        car = self.nearest_car_in_dock(self.cars)
        if car is None or car.cargo is not None: return None
        item=self.pop_next(kind)
        if item is None: return None
        car.cargo=item; return item
...
class Car:
    ...
    def set_exit_bays(self, exit_bays: List[ExitBay]):
        self.exit_bays=exit_bays

    def set_speed_mode(self, mode:str):
        if mode in SPEEDS: self.speed_mode=mode

    def stop(self):
        self.vx=self.vy=0.0; self.mode='manual'; self.goto=None

    def set_goto(self, p:Tuple[float,float]):
        self.goto=p; self.mode='goto'
        print(p)
    ...
    def unload(self) -> Optional[CargoItem]:
        """
        在任意位置卸载货物，返回被卸载的货物
        """
        cargo = self.cargo
        self.cargo = None
        bay_around = self.exit_bay_around(self.exit_bays)
...
class Scene:
    def __init__(self):
        ...
        for i,(x,y) in enumerate(spawn, start=1):
            car = Car(i,x,y)
            car.set_exit_bays([self.exitA, self.exitB])
            self.cars.append(car)

        # Transfer points
        self.arms: List[Arm]=[]
        self._add_arm('T1', base=(220, 130), dock=pygame.Rect(150, 170, 220, 80), count=3, phase=0)
        self._add_arm('T2', base=(W-220, H//2+40), dock=pygame.Rect(W-360, H//2-10, 220, 80), count=5, phase=1)
    ...
    def _add_arm(self, name:str, base:Tuple[int,int], dock:pygame.Rect, count:int, phase:int):
        kinds=fixed_AB_distribution(count, phase=phase)
        q=[CargoItem(name,i+1,kinds[i]) for i in range(count)]
        arm = Arm(name, base, dock, q)
    ...
    def handle_event(self, e):
        if e.type==pygame.KEYDOWN:
            ...
            if self.selected_group=='car':
                ...
                if e.key==pygame.K_d: car.unload()  # 添加卸载按键
            else:
    ...
    def _update_exit_logic(self, car:"Car", dt:float):
        ...
        for bay in (self.exitA, self.exitB):
            # if bay.rect.collidepoint(car.x, car.y):
            #     if car.cargo is not None and car.cargo.kind == bay.kind:
            #         self.delivered[bay.kind]+=1
            #         car.cargo=None
            #     rvx, rvy = -car.vx, -car.vy
            #     mag = math.hypot(rvx, rvy)
            #     if mag==0:
            #         rvx, rvy = 0, -1 if bay.name=='A' else 1
            #         mag=1
            #     car.vx = (rvx/mag)
            #     car.vy=(rvy/mag)
            #     car.heading = math.atan2(car.vy, car.vx)
            #     car.bounce_t = 0.6
            #     if bay.name=='A': car.y = bay.rect.bottom + CAR_R + 2
            #     else:             car.y = bay.rect.top - CAR_R - 2
            #     car.mode='manual'; car.goto=None
